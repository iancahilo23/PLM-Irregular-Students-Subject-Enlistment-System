...
let MAX_UNITS = 0; // ★ FIX 1: Initialize MAX_UNITS globally
...
document.addEventListener("DOMContentLoaded", () => {
    fetchStudentProfile();
    fetchMaxUnits(); // ★ FIX 2: Call the new function to get max units
    fetchSubjects();
    updateSummary();
    setupEventListeners();
});
...
function getStudentId() {
    return localStorage.getItem('PLM_Student_ID');
}

// --- NEW FUNCTION: FETCH MAX UNITS ---
async function fetchMaxUnits() {
    const studentId = getStudentId();
    if (!studentId) return;

    try {
        // Calls your server-side max_units.php to get the unit cap
        const response = await fetch(`max_units.php?student_id=${studentId}`);
        const data = await response.text(); // max_units.php returns a single number

        const maxUnits = parseInt(data.trim(), 10);

        if (!isNaN(maxUnits) && maxUnits >= 0) {
            MAX_UNITS = maxUnits; // Set the global variable
            console.log(`Max Units for current student: ${MAX_UNITS}`);
            updateSummary(); // Re-render summary with correct max units
        } else {
            console.error("Failed to parse max units:", data);
            MAX_UNITS = 24; // Fallback to a high limit if parsing fails
            updateSummary();
        }
    } catch (error) {
        console.error("Network or parsing error fetching max units:", error);
        MAX_UNITS = 24; // Fallback limit on network error
        updateSummary();
    }
}
// ------------------------------------
...
function toggleEnlist(uniqueId, event) {
    ...
    if (enlistedSubjects.includes(uniqueId)) {
        ...
        // ★ FIX 5: UNIT LIMIT CHECK START (Uses the globally loaded MAX_UNITS)
        if (MAX_UNITS === 0) {
            alert("Maximum unit limit is still loading. Please wait a moment and try again.");
            return;
        }
...
function updateSummary() {
    ...
    // ★ FIX 6: Use the helper function and display '...' if MAX_UNITS hasn't loaded yet
    const totalUnits = calculateCurrentTotalUnits();

    const displayMaxUnits = MAX_UNITS > 0 ? MAX_UNITS : '...';

    document.getElementById("total-units").innerText = `${totalUnits}/${displayMaxUnits}`;
